...
echo '<body>';
	...
	$games = $dao->getGameNames();

	echo '<h1>Hall of Fame</h1>';

	foreach ($games as $game) {

		echo '<h2>' . htmlspecialchars($game['gameName']) . '</h2>';
		...
		echo '<th>Game Started UTC</th>';
		echo '<th>Game Finished UTC</th>';
		echo '</tr>';
		echo "\r\n";

		$topTen = $dao->getTopTen(10, $game['gameName']);
		...
		foreach ($topTen as $entry) {
			...
			. '<td>' . ($entry['gameFinished'] != '0000-00-00 00:00:00' ? $entry['gameFinished'] : '') . '</td>'
			. '</tr>' . "\r\n";
		}
